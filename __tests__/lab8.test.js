describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let innerText1 = await innerText.jsonValue();
    expect(innerText1).toBe('Remove from Cart');
  }, 2500);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    let cartCount = await page.$('#cart-count');
    let innerText2 = await cartCount.getProperty('innerText');
    let innerText3 = await innerText2.jsonValue();
    expect(innerText3).toBe('20');
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let innerText = await button.getProperty('innerText');
      let innerText1 = await innerText.jsonValue();
      expect(innerText1).toBe('Add to Cart');
    }
    let cartCount = await page.$('#cart-count');
    let innerText2 = await cartCount.getProperty('innerText');
    let innerText3 = await innerText2.jsonValue();
    expect(innerText3).toBe('0');
  }, 10000);
